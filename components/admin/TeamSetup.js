  ...
  return (
    <div className="space-y-6">
        ...
        <div className="glass rounded-xl p-5 border border-white/10">
          ...
          <form onSubmit={handleCreateTeam} className="space-y-3">
            <div>
              ...
              <input type="text" value={newTeam.fullName} onChange={e => setNewTeam({ ...newTeam, fullName: e.target.value })} required className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-emerald-500" placeholder="e.g. Royal Challengers" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Short Name</label>
                <input type="text" value={newTeam.shortName} onChange={e => setNewTeam({ ...newTeam, shortName: e.target.value })} required className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-emerald-500" placeholder="e.g. RCB" />
              </div>
              <div>
                <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Department</label>
                <input type="text" value={newTeam.department} onChange={e => setNewTeam({ ...newTeam, department: e.target.value })} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-emerald-500" placeholder="Optional" />
              </div>
            </div>
            <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-medium py-2 rounded transition-colors flex items-center justify-center gap-2">
        ...
        <div className="glass rounded-xl p-5 border border-white/10">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={matchName} onChange={e => setMatchName(e.target.value)} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-blue-500" placeholder="e.g. Final Match / T20 Cup" />
            </div>
            <div>
              <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Team 1 (Name)</label>
              <select value={matchTeam1} onChange={e => setMatchTeam1(e.target.value)} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-blue-500">
            ...
            <div>
              ...
              <select value={matchTeam2} onChange={e => setMatchTeam2(e.target.value)} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-blue-500">
                <option value="">Select Team</option>
                {teams.map(t => <option key={t.id} value={t.id}>{t.fullName} ({t.shortName})</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={tossWinner} onChange={e => setTossWinner(e.target.value)} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-blue-500">
            ...
            <div>
              ...
              <select value={tossDecision} onChange={e => setTossDecision(e.target.value)} className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-blue-500">
                <option value="bat">Bat First</option>
                <option value="bowl">Bowl First</option>
              </select>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={handleStartMatch} className="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 rounded transition-colors">
                Initialize
              </button>
              <button onClick={handleUpdateToss} className="bg-white/10 hover:bg-white/20 text-white font-medium py-2 rounded transition-colors border border-white/10">
                Update Toss
              </button>
            </div>
          </div>
      ...
      <div className="glass rounded-xl p-5 border border-white/10">
        ...
        <div className="mb-4">
          <select value={selectedTeamId || ''} onChange={e => setSelectedTeamId(e.target.value)} className="w-full max-w-sm bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-purple-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="lg:col-span-1">
              <div className="bg-slate-800/30 border border-white/5 rounded-lg p-4">
                <h4 className="text-white text-sm font-semibold mb-3">Add New Player</h4>
                <form onSubmit={handleAddPlayer} className="space-y-3">
                  <div>
                    <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Player Name</label>
                    <input type="text" value={newPlayer.name} onChange={e => setNewPlayer({ ...newPlayer, name: e.target.value })} required className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-purple-500" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Jersey No.</label>
                    <input type="text" value={newPlayer.jersey} onChange={e => setNewPlayer({ ...newPlayer, jersey: e.target.value })} required className="w-full bg-slate-800/50 border border-white/10 rounded px-3 py-2 text-white outline-none focus:border-purple-500" />
                  </div>
                  <button type="submit" className="w-full bg-slate-700 hover:bg-slate-600 text-white font-medium py-2 rounded transition-colors flex items-center justify-center gap-2">
            ...
            <div className="lg:col-span-2">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {teams.find(t => t.id === selectedTeamId)?.players.map(p => (
                  <div key={p.id} className="bg-slate-800/80 border border-white/5 rounded p-3 flex items-center justify-between group">
                    <div>
                      <div className="text-white font-medium text-sm">{p.name}</div>
                      <div className="text-slate-400 text-xs">#{p.jersey}</div>
                    </div>
                    <button onClick={() => handleDeletePlayer(selectedTeamId, p.id)} className="text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-red-500/20 rounded">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
