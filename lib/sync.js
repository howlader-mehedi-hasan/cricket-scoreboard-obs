import Peer from 'peerjs';
import { v4 as uuidv4 } from 'uuid';

class SyncManager {
  constructor() {
    this.peer = null;
    this.connections = [];
    this.role = null; // 'host' or 'client'
    this.hostId = null;
    this.state = {
      match: null,
      style: null
    };
    this.listeners = new Set();
    this.history = [];
    this.socket = null;
    this.onReadyCallback = null;
  }

  setSocket(socket) {
    this.socket = socket;
  }

  // Initialize as Host
  async initHost(savedId = null) {
    this.role = 'host';
    const peerId = savedId || `crick-host-${uuidv4().slice(0, 8)}`;
    
    return new Promise((resolve) => {
      this.peer = new Peer(peerId);

      this.peer.on('open', (id) => {
        console.log('[Sync] Host ID:', id);
        this.hostId = id;
        resolve(id);
      });

      this.peer.on('connection', (conn) => {
        console.log('[Sync] New connection:', conn.peer);
        this.setupConnection(conn);
      });
    });
  }

  // Initialize as Client
  async initClient(hostId) {
    this.role = 'client';
    this.hostId = hostId;

    return new Promise((resolve, reject) => {
      this.peer = new Peer();

      this.peer.on('open', () => {
        const conn = this.peer.connect(hostId);
        this.setupConnection(conn);
        
        conn.on('open', () => {
          console.log('[Sync] Connected to host:', hostId);
          resolve();
        });

        conn.on('error', (err) => reject(err));
      });
    });
  }

  setupConnection(conn) {
    this.connections.push(conn);

    conn.on('data', (data) => {
      if (data.type === 'STATE_UPDATE') {
        this.state = data.payload;
        this.notify();
      } else if (data.type === 'COMMAND') {
        if (this.role === 'host') {
          this.handleCommand(data.payload);
        }
      }
    });

    conn.on('close', () => {
      this.connections = this.connections.filter(c => c !== conn);
      console.log('[Sync] Connection closed');
    });

    // If host, send initial state
    if (this.role === 'host') {
      setTimeout(() => {
        conn.send({ type: 'STATE_UPDATE', payload: this.state });
      }, 500);
    }
  }

  // Host only: Save to broadcast and notify
  saveAndBroadcast() {
    this.connections.forEach(conn => {
      if (conn.open) {
        conn.send({ type: 'STATE_UPDATE', payload: this.state });
      }
    });
    this.notify();
  }

  // Host only: Handle commands from clients
  handleCommand({ action, data }) {
    if (action === 'match:undo') {
      const lastState = this.history.pop();
      if (lastState) {
        this.state.match = JSON.parse(lastState);
        this.saveAndBroadcast();
      }
      return;
    }

    // Save history before updates
    this.history.push(JSON.stringify(this.state.match));
    if (this.history.length > 50) this.history.shift();

    // Initialize state if null
    if (!this.state.match) this.state.match = this.getDefaultMatch();
    if (!this.state.style) this.state.style = this.getDefaultStyle();

    let newMatch = { ...this.state.match };
    let newStyle = { ...this.state.style };

    if (action === 'match:update') {
      newMatch[data.field] = data.value;
    } else if (action === 'match:updateBulk') {
      data.updates.forEach(({ field, value }) => {
        newMatch[field] = value;
      });
    } else if (action === 'match:addBall') {
      let recent = JSON.parse(newMatch.recent_balls || '[]');
      recent.push(data.ball);
      if (recent.length > 36) recent = recent.slice(-36);
      newMatch.recent_balls = JSON.stringify(recent);

      let fullLog = JSON.parse(newMatch.ball_log || '[]');
      fullLog.push(data.ball);
      newMatch.ball_log = JSON.stringify(fullLog);
    } else if (action === 'match:recordBall') {
      if (data.updates) {
        data.updates.forEach(({ field, value }) => {
          newMatch[field] = value;
        });
      }
      if (data.ball) {
        let recent = JSON.parse(newMatch.recent_balls || '[]');
        recent.push(data.ball);
        if (recent.length > 36) recent = recent.slice(-36);
        newMatch.recent_balls = JSON.stringify(recent);

        let fullLog = JSON.parse(newMatch.ball_log || '[]');
        fullLog.push(data.ball);
        newMatch.ball_log = JSON.stringify(fullLog);
      }
    } else if (action === 'style:update') {
      newStyle[data.field] = data.value;
    }

    this.state = {
      match: newMatch,
      style: newStyle
    };

    this.saveAndBroadcast();
  }

  // Emit an action (works for both Host and Client)
  emit(action, data) {
    if (this.role === 'host') {
      this.handleCommand({ action, data });
    } else {
      // Primary: Send via P2P for low latency
      const conn = this.connections[0];
      if (conn && conn.open) {
        conn.send({ type: 'COMMAND', payload: { action, data } });
      }
      
      // Fallback/Sync: Send via Socket.IO to the server
      if (this.socket) {
        this.socket.emit(action, data);
      }
    }
  }

  subscribe(listener) {
    this.listeners.add(listener);
    listener(this.state);
    return () => this.listeners.delete(listener);
  }

  notify(fromServer = false) {
    this.listeners.forEach(l => l(this.state));

    // Broadcast to server if we updated locally
    if (!fromServer && this.socket && this.state.match) {
      this.socket.emit('match:updateBulk', { 
        updates: Object.entries(this.state.match).map(([field, value]) => ({ field, value }))
      });
      if (this.state.style) {
        Object.entries(this.state.style).forEach(([field, value]) => {
          this.socket.emit('style:update', { field, value });
        });
      }
    }
  }

  getDefaultMatch() {
    return {
      team1_name: 'Team A', team2_name: 'Team B',
      runs: '0', wickets: '0', overs: '0', balls: '0',
      innings: '1', striker_name: 'Batsman 1', striker_runs: '0', striker_balls: '0',
      non_striker_name: 'Batsman 2', non_striker_runs: '0', non_striker_balls: '0',
      bowler_name: 'Bowler 1', bowler_overs: '0', bowler_runs: '0', bowler_wickets: '0',
      recent_balls: '[]', ball_log: '[]', batting_team: 'team1', match_status: 'Match Start'
    };
  }

  getDefaultStyle() {
    return {
      active_profile: 'default style',
      layout_type: 'default',
      bar_width: '90',
      bar_height: '10',
      bar_x: '5',
      bar_y: '85',
      theme_color: '#1e293b'
    };
  }
}

// Singleton instance
const syncManager = typeof window !== 'undefined' ? new SyncManager() : null;
export default syncManager;
