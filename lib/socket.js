  ...
  useEffect(() => {
    ...
    socket.on('match:state', (data) => {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        syncManager.notify(true); // pass true to avoid infinite loop of emitting back to server
      }
    });

    socket.on('style:state', (data) => {
      console.log('[Socket.IO] Received style state:', data);
      if (data && Object.keys(data).length > 0) {
        syncManager.state.style = data;
        syncManager.notify(true);
      }
    });
